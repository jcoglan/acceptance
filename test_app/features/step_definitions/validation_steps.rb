...
Given /^the (\S+) class validates length of (\S+) \((\S+): (\S+)\) on (\S+)$/ do |class_name, field, option, value, event|
  inject_code class_name, "validates_length_of :#{field}, :#{option} => #{value}, :on => :#{event}"
end
